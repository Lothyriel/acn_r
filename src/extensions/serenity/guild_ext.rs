...
pub trait GuildExt {
    ...
    fn get_voice_states(self, cache: Arc<Cache>) -> Result<HashMap<UserId, VoiceState>>;
}
...
impl GuildExt for GuildId {
    ...
    fn get_voice_states(self, cache: Arc<Cache>) -> Result<HashMap<UserId, VoiceState>> {
        let guild = cache
            .guild(self)
            .ok_or_else(|| anyhow!("Couldn't get guild {} from cache", self.get()))?;
...
pub async fn get_all_online_users(
    http: Arc<Http>,
    cache: Arc<Cache>,
) -> Result<impl Iterator<Item = StatusInfo>> {
    ...
    let get_online_users_results = guilds_info.into_iter().map(move |g| {
        ...
        let online_users = voice_states
            ...
            .map(move |c| StatusInfo::new(c.user_id.get(), g.id.get()));
